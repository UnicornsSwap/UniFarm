...
contract UNI_FARM_USDT is Auth {
  ...
  function claim() public {
    ...
    if (uniToken.balanceOf(address(this)) == 0) {
      leave();
      return;
    }
    uint claimableUni = getClaimableUni(farmer);
    uniToken.transfer(msg.sender, claimableUni);
    farmer.lastClaimedBlock = block.number;
    emit Claimed(msg.sender, claimableUni);
  }
  ...
  function uniUsdt(uint _uniUsdt) onlyTrigger public {
    uniUsdt = _uniUsdt;
  }

  function getClaimableUni(Farmer _farmer) private view returns (uint) {
    ...
    uint claimableUni = claimableUsdt.mul(decimal6).div(uniUsdt);
    return Math.min(claimableUni, uniToken.balanceOf(address(this)));
  }
}
